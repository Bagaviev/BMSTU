...
sample['STHST_k'], sample['STHST_d'] = ta.STOCH(sample['HIGH'], sample['LOW'], sample['CLOSE'], fastk_period=14,
...
title = "Total recommendations for '{}' to trade: {}".format(
    sample.TRADEDATE.tail(1).to_string(index=False).replace(' ', ''),
    sample.Recommendation.tail(1).to_string(index=False).replace(' ', ''))
# 4================================================================================================================================
year = sample
...
def plotter(period, period_name):
    ...
    recommendation = period.tail(3).plot(ylim=[-1.1, 1.1], x='TRADEDATE', y='Stance_st', figsize=(5, 2), grid=True,
                                         title=title)
...
year.to_csv('D:\Desktop_D\УЧЕБА\MAGA_IU5\sem4\diploma_shit\Exchange_Web\static\data\parsed_EURUSD_y.csv', sep=';',
            encoding='utf-8')
